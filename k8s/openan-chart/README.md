...
### Ingress Controller 部署方案

支持两种 Ingress Controller 部署方案，可通过 `ingressController` 配置选择：

#### 方案一：NodePort（开发测试环境）

适用于开发测试环境，通过节点 IP + 端口访问：

```bash
# 部署 Ingress Controller（NodePort 模式）
helm install openan . \
  --namespace openan \
  --set createNamespace=false \
  --set ingressController.enabled=true \
  --set ingressController.serviceType=NodePort \
  --set ingressController.nodePort.http=30083 \
  --set ingress.host=openan.local

# 访问方式：http://<节点IP>:30083
# 需要在 /etc/hosts 添加：
# <节点IP> openan.local
```

#### 方案二：LoadBalancer（生产环境）

适用于生产环境，通过负载均衡器 IP 访问：

```bash
# 部署 Ingress Controller（LoadBalancer 模式）
helm install openan . \
  --namespace openan \
  --set createNamespace=false \
  --set ingressController.enabled=true \
  --set ingressController.serviceType=LoadBalancer \
  --set ingress.host=openan.local

# 查看 LoadBalancer IP
kubectl get svc ingress-nginx-controller -n openan

# 访问方式：http://<LoadBalancer IP>
# 需要在 DNS 或 /etc/hosts 添加：
# <LoadBalancer IP> openan.local
```

#### 使用集群已有的 Ingress Controller

如果集群已经部署了 Ingress Controller（如 ingress-nginx），可以不部署新的：

```bash
# 不部署 Ingress Controller，使用集群已有的
helm install openan . \
  --namespace openan \
  --set createNamespace=false \
