...
| `negotiation_request` | 协商请求发送 | 引擎发起 A2A-T 协商时 | `agent`, `negotiation_id` |
| `negotiation_resolved` | 协商达成一致 | 协商达成协议时 | `agent`, `negotiation_id`, `resolution` |
| `negotiation_failed` | 协商失败 | 协商被拒绝或超时时 | `agent`, `negotiation_id`, `reason` |
